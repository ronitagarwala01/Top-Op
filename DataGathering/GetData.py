    ...
    for fileName in filesToPull:
        if(fileName.index('.csv.npz') < 0):
            ...
            try:
                ...
                dataPoints.append([xPhys,forces,degreesOfFreedom,shape,compliance_max,compliance])
...
def getAgents(path,numberToGrab):
    """
    ...
    numberToGrab = min(numberToGrab,max_data_points)
    print("Number of data points: {}".format(len(dir_list)))
    print("Getting {} datapoints.".format(numberToGrab))


    data = []
    for i in range(numberToGrab):
        agent = getAgent(os.path.join(path,dir_list[i]))
        for xPhys,forces,degreesOfFreedom,shape,compliance_max,compliance in agent:
            data.append([xPhys,forces,degreesOfFreedom,shape,compliance_max,compliance])
