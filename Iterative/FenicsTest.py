...
def fenicsOptimizer(problemConditions,part:np.ndarray):
    ...
    def main():
        iter_sol = Function(X)
        iter_sol.vector()[:] = part
        (f, u) = forward(iter_sol)                 # Forward problem
        ...
        FX_ = Constant((1,0))
        FY_ = Constant((0,1))
        FX = assemble(inner(f, FX_)*dx)
        FY = assemble(inner(f, FY_)*dx)
        print("FX: ", FX)
        print("FY: ", FY)
