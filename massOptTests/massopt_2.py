...
# E_void = Constant(1.0e+22)                     # Young Modulus for Voids
...
# sub_mesh = SubMesh(mesh, domains, 0)
...
# f = PointLoad(domains, pt=(x_1, y_1), vl=(0.0, 5000000.0), tol=1e-1,degree = 1)
...
for i in range(x_.shape[0]):
    if domains.array()[i] == 1 or domains.array()[i] == 2 or domains.array()[i] == 3:
        ...
        E.vector().vec().setValueLocal(i, Constant(2.0e+11))
...
for i in range(f_.shape[0]):
    if domains.array()[i] == 1:
        ...
        f.vector().vec().setValueLocal(2*i+1, 5000000.0)
...
def helmholtz_filter(rho_n):
      ...
      a = (r**2)*inner(grad(rho), grad(w))*dx(0) + rho*w*dx(0) + rho*w*dx(1) + rho*w*dx(2) + rho*w*dx(3)
      L = rho_n*w*dx
...
def forward(x):
    ...
    a = simp(rho)*inner(sigma(u), strain(v))*dx# + (inner(sigma_void(u), strain(v))*dx(1))
    L = dot(f, v)*dx#+dot(f, v)*dx(1)+dot(f, v)*dx(2)+dot(f, v)*dx(3)# + dot(f2, v)*dx + dot(f3, v)*dx# + dot(-f, v)*ds(3) + dot(f, v)*ds(4)
    u = Function(U)
    solve(a == L, u)
    return u  
...
if __name__ == "__main__":
    x = interpolate(Constant(1.0), X)  # Initial value of 0.5 for each element's density
    # print(x.vector()[:].shape)
    print(x_.shape)
    print(f_.shape)
    print(f_)
    print(domains.array().shape)
    print(domains.array())
    # u_ = Function(U)
    print(f.vector()[:])
    ...
    class ExtensionConstraint(InequalityConstraint):
        ...
        def jacobian(self, m):
            ...
            for (index, Coordinate) in zip(V2D, Coordinates):
