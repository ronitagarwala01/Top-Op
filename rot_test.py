...
E_rot = Constant(1.0e+22)

lmbda_rot = (E_rot*nu) / ((1.0 + nu)*(1.0 - (2.0*nu)))  # Lame's first parameter
G_rot = E_rot / (2.0*(1.0 + nu))                        # Lame's second parameter / Shear Modulus
...
for i in range(f_.shape[0]):
    if domains.array()[i] == 1:
        f.vector().vec().setValueLocal(2*i, -15000000.0)
        f.vector().vec().setValueLocal(2*i+1, -1000000.0)
    elif domains.array()[i] == 2:
        f.vector().vec().setValueLocal(2*i, 0.0)
        f.vector().vec().setValueLocal(2*i+1, +2000000.0)
...
def sigma_rot(u):
    return lmbda_rot*nabla_div(u)*Identity(2) + 2*G_rot*strain(u)
...
#     u_rot = TrialFunction(U)  # Trial Function
#     v_rot = TestFunction(U)   # Test Function
#     a = simp(rho)*inner(sigma_rot(u_rot), strain(v_rot))*dx
#     L = dot(f, v_rot)*dx
#     u_rot = Function(U)
#     solve(a == L, u_rot)
#     return u_rot
