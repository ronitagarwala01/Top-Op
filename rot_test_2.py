...
U_ele = VectorElement("Lagrange", mesh.ufl_cell(), 2)
V_ele = VectorElement("R", mesh.ufl_cell(), 0)
W = FunctionSpace(mesh, MixedElement(U_ele, V_ele))
...
# Rotation Null Space Basis Vectors
Z = Expression(('-x[1]', 'x[0]'), degree=1)
# Z_func = interpolate(Z, U)
